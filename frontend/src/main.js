...
function renderApp() {
    ...
    <style>
        ...
        /* Scrollbar personalizado para la ventana modal */
        .auth-card::-webkit-scrollbar { width: 8px; }
        .auth-card::-webkit-scrollbar-track { background: rgba(255,255,255,0.1); border-radius: 4px; }
        .auth-card::-webkit-scrollbar-thumb { background: #6c5ce7; border-radius: 4px; }
        .auth-card::-webkit-scrollbar-thumb:hover { background: #a29bfe; }
    </style>
  `;
    ...
    <section id="respond-section" class="hidden" style="position: fixed; top:0; left:0; width:100%; height:100%; background: rgba(0,0,0,0.85); backdrop-filter: blur(5px); display: flex; justify-content: center; align-items: center; z-index: 99;">
        <div class="auth-card" style="width: 100%; max-width: 600px; max-height: 90vh; overflow-y: auto; background: #2d3436; padding: 30px; border-radius: 20px; border: 1px solid #444;">
...
// ==========================================
// 5. FUNCIÓN CORREGIDA PARA CREAR JUEGO
// ==========================================
window.createNewGameQuick = async function() {
    ...
    const btn = document.querySelector('button[onclick="window.createNewGameQuick()"]');
    ...
    try {
        const res = await fetch(`${API_URL}/games`, {
            ...
            body: JSON.stringify({ titulo: titulo, descripcion: "Creado desde encuesta", categoriaNombre: cat })
        });
...
function showLogin() { hideAll(); document.getElementById('auth-section').classList.remove('hidden'); document.getElementById('login-container').classList.remove('hidden'); document.getElementById('register-container').classList.add('hidden'); }
function showRegister() { document.getElementById('auth-section').classList.remove('hidden'); document.getElementById('login-container').classList.add('hidden'); document.getElementById('register-container').classList.remove('hidden'); }
...
function checkAuth() { const t=localStorage.getItem('token'); const u=localStorage.getItem('user'); if(t&&u){ try{ currentUser=JSON.parse(u); window.showDashboard(); } catch(e){ handleLogout(); } } else showLogin(); }
async function handleLogin(e) { e.preventDefault(); const res=await fetch(`${API_URL}/auth/login`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email:document.getElementById('login-email').value, password:document.getElementById('login-password').value})}); if(res.ok){ const d=await res.json(); if(d.token){ localStorage.setItem('token',d.token); const usuarioAdaptado = { nombre: d.username || (d.role.includes('CRAFTER') ? 'Admin' : 'Player'), role: d.role, email: document.getElementById('login-email').value }; localStorage.setItem('user',JSON.stringify(usuarioAdaptado)); currentUser=usuarioAdaptado; window.showDashboard(); } } else alert('Error credenciales'); }
